...
def page_activiteiten():
    ...
    if current_role() == "admin":
        ...
        if df_del.empty: st.caption("Geen toekomstige activiteiten.")
            ...
            st.divider(); st.subheader("Activiteiten bewerken / verwijderen")
            # Actie kiezen via titelbalk-knoppen (start: leeg)
            hc1, hc2, hc3 = st.columns([6, 1.2, 1.4])
            with hc1:
                st.caption("Kies eerst een actie en selecteer daarna de activiteit(en).")
            with hc2:
                if st.button("bewerken", key="act_hdr_edit"):
                    st.session_state.pop("del_act_sel", None)
                    st.session_state.pop("mg_edit_select", None)
                    st.session_state["act_admin_action"] = "Bewerken"
            with hc3:
                if st.button("verwijderen", key="act_hdr_del"):
                    st.session_state.pop("del_act_sel", None)
                    st.session_state.pop("mg_edit_select", None)
                    st.session_state["act_admin_action"] = "Verwijderen"

            _act_action = st.session_state.get("act_admin_action")
                ...
                if st.button("Verwijder geselecteerde activiteiten", key="del_act_btn", type="primary", disabled=is_readonly() or not sel_labels):
                    ...
                    if not ids: st.warning("Niets geselecteerd.")
                        ...
                        try:
                            ...
                            st.success(f"Verwijderd: {len(ids)} activiteit(en)."); st.session_state.pop("del_act_sel", None); st.session_state.pop("act_admin_action", None); st.rerun()
                        except Exception as e:
                            st.error(f"Verwijderen mislukt: {e}")
            else:
                _edit_choice = st.selectbox("Kies activiteit om te bewerken", ["— kies —"] + options, index=0, key="mg_edit_select")
                ...
                if _act_id:
                    ...
                    if st.button("Opslaan wijzigingen", key="adm_e_save", type="primary", disabled=is_readonly()):
                        try:
                            ...
                            st.success("Activiteit bijgewerkt."); st.session_state.pop("mg_edit_select", None); st.session_state.pop("act_admin_action", None); st.rerun()
                        except Exception as ex:
                            st.error(f"Opslaan mislukt: {ex}")
