...
def page_profiel():
    ...
    with col2:
        if ("telefoon" in _df_cols) or v_tel:
                        # Telefoon opgesplitst: landcode + nummer
                        _cc = ""
                        _digits = ""
                        try:
                            _cc, _digits = _parse_phone(v_tel)
                        except Exception:
                            _cc, _digits = "", ""
                        colp1, colp2 = st.columns([0.35, 0.65])
                        with colp1:
                            i_tel_cc = st.text_input("Landcode", value=_cc or "+32", key="prof_tel_cc")
                        with colp2:
                            i_tel_num = st.text_input("Nummer (bv. 476591372)", value=_digits, key="prof_tel_num")
        else:
            i_tel = None
        if ("adres" in _df_cols) or v_adres:
                        # Adres opgesplitst in 3 lijnen
                        _ap = _parse_adres_to_parts(v_adres)
                        st.markdown("**Adres**")
                        cola1, cola2, cola3 = st.columns([0.5, 0.25, 0.25])
                        with cola1:
                            i_straat = st.text_input("Straat", value=_ap.get("straat",""), key="prof_adr_straat")
                        with cola2:
                            i_nummer = st.text_input("Nummer", value=_ap.get("nummer",""), key="prof_adr_nummer")
                        with cola3:
                            i_bus = st.text_input("Bus", value=_ap.get("bus",""), key="prof_adr_bus")
                        colb1, colb2 = st.columns([0.3, 0.7])
                        with colb1:
                            i_postcode = st.text_input("Postcode", value=_ap.get("postcode",""), key="prof_adr_pc")
                        with colb2:
                            i_stad = st.text_input("Stad", value=_ap.get("stad",""), key="prof_adr_stad")
                        i_land = st.text_input("Land", value=_ap.get("land",""), key="prof_adr_land")
        else:
            i_adres = None
    ...
    if st.button("Bewaar mijn gegevens", type="primary", disabled=disabled, key="prof_save_btn"):
        try:
            # Format telefoon & adres
            try:
                tel_fmt = _format_phone_bel_style(i_tel_cc, i_tel_num) if ('i_tel_cc' in locals() and 'i_tel_num' in locals()) else None
            except Exception:
                tel_fmt = None
            try:
                adr_fmt = _format_adres_from_parts(i_straat, i_nummer, i_bus, i_postcode, i_stad, i_land)
            except Exception:
                adr_fmt = None
            payload = {
                "username": uname,
