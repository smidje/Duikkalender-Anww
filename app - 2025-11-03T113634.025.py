...
# Persistent edit-mode: open editor and keep it open until saved/cancelled
if st.session_state.get("mg_edit_mode") and st.session_state.get("mg_edit_id") == _act_id:
        ...
        st.rerun()
# Als selectie wijzigt tijdens edit, sluit editor
if st.session_state.get("mg_edit_mode") and st.session_state.get("mg_edit_id") != _act_id:
    ...
    st.session_state.pop("mg_edit_id", None)
            ...
            if clicked_edit:
                if not _act_id:
                    ...
                    st.session_state['act_admin_mode'] = 'edit'
                    st.session_state['act_admin_edit_id'] = _act_id
            ...
            if clicked_del:
                if not _act_id:
                    ...
                    try:
                        ...
                        for k in ['mg_edit_select','act_admin_mode','act_admin_edit_id','mg__last_act_id']:
                            st.session_state.pop(k, None)
                        for _k in list(st.session_state.keys()):
                            if _k.startswith('adm_e_'): del st.session_state[_k]
                        st.rerun()
                    except Exception as e:
                        st.error(f"Verwijderen mislukt: {e}")

            open_edit = (
                st.session_state.get('act_admin_mode') == 'edit' and
                st.session_state.get('act_admin_edit_id') == _act_id and
                _act_id is not None
            )

            _last_id = st.session_state.get('mg__last_act_id')
            if _last_id != _act_id:
                try:
                    for _k in list(st.session_state.keys()):
                        if _k.startswith('adm_e_'): del st.session_state[_k]
                    ...
                    pass
                st.session_state['mg__last_act_id'] = _act_id
            ...
            if open_edit:
                ...
                organisator_user2 = _org_map2.get(organisator_sel2)
                if st.button("Opslaan wijzigingen", key=f"adm_e_save_{_act_id}", type="primary", disabled=is_readonly()):
                    try:
                        payload = {
                            "titel": etitel.strip(),
                            "omschrijving": (eomschr or "").strip(),
                        ...
                        st.error(f"Opslaan mislukt: {ex}")
        if st.button("Annuleren", key=f"adm_e_cancel_{\_act_id}"):
            st.session_state.pop("mg_edit_mode", None)
            st.session_state.pop("mg_edit_id", None)
            st.session_state.pop("mg_edit_select", None)
            st.session_state.pop("mg__last_act_id", None)
            st.rerun()
