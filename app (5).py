...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        with col_main:
            # Kleurstrook per activiteit (visuele onderscheiding)
            try:
                _hex = activity_color_get(row.get('id'))
                if _hex:
                    st.markdown(f"<div style='background:{_hex};height:14px;border-radius:6px;margin:4px 0;'></div>", unsafe_allow_html=True)
            except Exception:
                pass
