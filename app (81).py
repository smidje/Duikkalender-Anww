...
# Secrets -> env fallback for Supabase service REST
try:
    import os as _os
    _os.environ.setdefault('SUPABASE_URL', st.secrets.get('SUPABASE_URL',''))
    _os.environ.setdefault('SUPABASE_SERVICE_KEY', st.secrets.get('SUPABASE_SERVICE_KEY',''))
except Exception:
    pass
import requests
import pandas as pd
...
import json
import base64
# --- Banner helpers (SERVICE-ROLE REST first; DB + session fallback) ---
def _sb_rest_conf():
...
def _asset_rest_upsert(key: str, value_text: str):
    ...
    if not (url and sk):
        return False
    payload = {'key': key, 'image_b64': value_text, 'updated_by': None, 'updated_ts': __import__('datetime').datetime.utcnow().isoformat()}
    try:
        r = httpx.post(
            f'{url}/rest/v1/app_assets',
            json=payload,
...
def asset_set_bytes(key: str, data: bytes | None, updated_by: str | None = None):
    ...
    try:
        ...
        st.session_state[f'_asset_{key}'] = data
