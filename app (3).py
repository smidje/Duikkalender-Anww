...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        with col_main:
            # Kleurstrook op basis van soort activiteit

            try:

                _map = activity_type_colors_get()

                _kind = activity_kind_from_title(row.get("titel") or "", _map)

                _hex = _map.get(_kind) if _kind else None

                if _hex:

                    st.markdown(f"<div style='background:{_hex};height:6px;border-radius:6px;margin:4px 0;'></div>", unsafe_allow_html=True)
...
def page_beheer():
    ...
    # ---- Activiteitstypes & kleur (beheer) ----
    st.divider(); st.subheader("Soorten activiteiten & kleur")
    try:
        mapping = activity_type_colors_get()
    except Exception:
        mapping = {"Cursus":"#E35B40","Clubduik":"#684EF5","Vergadering":"#F5994E","Groepsactiviteit":"#DE9AED"}
    if mapping:
        view = pd.DataFrame([{"Soort": k, "Kleur": v} for k, v in mapping.items()])
        st.dataframe(view, use_container_width=True, hide_index=True)
    with st.form("act_types_form"):
        st.caption("Toevoegen of wijzigen")
        c1, c2, c3 = st.columns([2,2,2])
        with c1:
            new_kind = st.text_input("Soort (bv. Cursus)")
        with c2:
            new_color = st.color_picker("Kleur", value=(mapping.get(new_kind) if new_kind in mapping else "#E35B40"))
        with c3:
            remove_kind = st.text_input("Verwijder soort (exacte naam)")
        save = st.form_submit_button("Bewaar wijzigingen", disabled=is_readonly())
    if save:
        try:
            if remove_kind and remove_kind in mapping:
                mapping.pop(remove_kind, None)
            if new_kind:
                mapping[new_kind] = new_color
            activity_type_colors_set(mapping)
            st.success("Activiteitstypes bijgewerkt."); st.rerun()
        except Exception as e:
            st.error(f"Opslaan mislukt: {e}")
...
# --- Activiteitstypes & kleuren (instellingen via asset meta) ---
def activity_type_colors_get() -> dict:
    try:
        m = asset_meta_get('activity_type_colors') or {}
        if not isinstance(m, dict):
            m = {}
    except Exception:
        m = {}
    defaults = {"Cursus": "#E35B40", "Clubduik": "#684EF5", "Vergadering": "#F5994E", "Groepsactiviteit": "#DE9AED"}
    for k, v in defaults.items():
        if k not in m or not m.get(k):
            m[k] = v
    z = {}
    for k, v in m.items():
        x = str(v or "").strip()
...
def activity_type_colors_set(mapping: dict):
    if is_readonly():
        return
    out = {}
    for k, v in (mapping or {}).items():
        ...
        if x and not x.startswith("#"):
            x = "#" + x
        out[str(k)] = x
    asset_meta_set('activity_type_colors', out)

def activity_kind_from_title(title: str, mapping: dict) -> str | None:
    t = (title or "").strip()
    if not t:
        return None
    t0 = t.split(":")[0].strip()
    for k in (mapping or {}).keys():
        if t0.lower().startswith(str(k).lower()):
            return k
    return None
