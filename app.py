...
def page_leden():
    """Alle leden tonen (alleen lezen) voor users & members."""
    st.header("Leden")
    try:
        df = leden_list_df()
    except Exception as e:
...
# --- Helpers: adres & telefoon parsing/formattering ---
...
def page_ledenbeheer():
    ...
    with st.form("leden_form", clear_on_submit=True):
        ...
        # Contact — telefoon (landcode + nummer)
        st.markdown("**Telefoon**")
        colp1, colp2 = st.columns([0.35, 0.65])
        with colp1:
            adm_tel_cc = st.text_input("Landcode", value="+32", key="adm_tel_cc")
        with colp2:
            adm_tel_num = st.text_input("Nummer (bv. 476591372)", value="", key="adm_tel_num")
        # Adres — 3 lijnen
        st.markdown("**Adres**")
        cola1, cola2, cola3 = st.columns([0.5, 0.25, 0.25])
        with cola1:
            adm_straat = st.text_input("Straat", value="", key="adm_adr_straat")
        with cola2:
            adm_nummer = st.text_input("Nummer", value="", key="adm_adr_nummer")
        with cola3:
            adm_bus = st.text_input("Bus", value="", key="adm_adr_bus")
        colb1, colb2 = st.columns([0.3, 0.7])
        with colb1:
            adm_postcode = st.text_input("Postcode", value="", key="adm_adr_pc")
        with colb2:
            adm_stad = st.text_input("Stad", value="", key="adm_adr_stad")
        adm_land = st.text_input("Land", value="België", key="adm_adr_land")
    ...
    if submitted:
        # Format telefoon & adres voor opslag
        try:
            tel_fmt = _format_phone_bel_style(adm_tel_cc, adm_tel_num) if ('adm_tel_cc' in locals() and 'adm_tel_num' in locals()) else None
        except Exception:
            tel_fmt = None
        try:
            adr_fmt = _format_adres_from_parts(adm_straat, adm_nummer, adm_bus, adm_postcode, adm_stad, adm_land)
        except Exception:
            adr_fmt = None
        if not username:
        ...
        payload = {
            "telefoon": tel_fmt, "adres": adr_fmt, "email": email_eff, "voornaam": (vn or "").strip(), "achternaam": (an or "").strip(),
...
def weekmail_test_send(to_email: str):
    """Call Supabase Edge Function 'weekmail' in dry-run mode to send only to 'to_email'."""
    try:
        base_url = SUPABASE_URL if 'SUPABASE_URL' in globals() else os.getenv('SUPABASE_URL', '')
        anon_key = SUPABASE_ANON_KEY if 'SUPABASE_ANON_KEY' in globals() else os.getenv('SUPABASE_ANON_KEY', '')
        ...
        with col_main:
            with st.expander(f"{titel}   ·   {badge}", expanded=False):
                ...
                try:
                    ...
                    # Te water-uur zoeken in omschrijving
                    import re as __re
                    ...
                    try:
                        ...
                        _tw = ""
                    # Organisator tonen als 'Voornaam Achternaam (username)' indien gekend
                    ...
                    try:
                        ...
                        if _org_u and isinstance(_leden_df, pd.DataFrame) and not _leden_df.empty and "username" in _leden_df.columns:
                            ...
                            if not _r.empty:
                                _vn = (_r.iloc[0].get("voornaam") or "").strip()
                                _an = (_r.iloc[0].get("achternaam") or "").strip()
                                _org_disp = f"{_vn} {_an} ({_org_u})".strip()
                    except Exception:
                        pass
                    _meta_df = pd.DataFrame([{"Datum": _datum_str, "Titel": _titel_x, "Locatie": _loc_x, "Te water": _tw, "Organisator": _org_disp}])
                
                    # Deelnemers die ingeschreven hebben (status == yes), met brevet + eten info
                    _s_exp = s.copy() if isinstance(s, pd.DataFrame) else pd.DataFrame([])
                    if not _s_exp.empty and "status" in _s_exp.columns:
                        _s_exp = _s_exp[_s_exp["status"] == "yes"].copy()
                    else:
                        _s_exp = pd.DataFrame([])
                    # Merge met leden om voornaam/achternaam/brevet te hebben
                    try:
                        _leden = leden_list_df()
                    except Exception:
                        _leden = pd.DataFrame([])
                    if not _s_exp.empty and isinstance(_leden, pd.DataFrame) and not _leden.empty:
                        _keep = [c for c in ["username","id","voornaam","achternaam","duikbrevet"] if c in _leden.columns]
                        _leden1 = _leden[_keep].copy()
                        _s_exp = _s_exp.merge(_leden1, on="username", how="left")
                        # fallback via lid_id
                        if (("voornaam" not in _s_exp.columns) or _s_exp["voornaam"].isna().all()) and "id" in _leden.columns:
                            _leden2 = _leden.rename(columns={"id":"lid_id"})[[c for c in ["lid_id","voornaam","achternaam","duikbrevet"] if c in _leden.columns]]
                            _s_exp = _s_exp.merge(_leden2, on="lid_id", how="left", suffixes=("", "_lid"))
                    # Parse extra mee-eters uit opmerking
                    def __extra_people(x):
                        try:
                            ...
                            return 0
                    _extra_vals = _s_exp["opmerking"].apply(__extra_people) if "opmerking" in _s_exp.columns else 0
                    _eat_vals = _s_exp["eating"].apply(lambda v: bool(v) if pd.notna(v) else False) if "eating" in _s_exp.columns else False
                    _meal_vals = _s_exp["meal_choice"] if "meal_choice" in _s_exp.columns else ""
                    # Normaliseer brevetten (canonieke volgorde)
                    _brevet = _s_exp["duikbrevet"].apply(canon_brevet) if "duikbrevet" in _s_exp.columns else None
                    _vn = _s_exp["voornaam"] if "voornaam" in _s_exp.columns else ""
                    _an = _s_exp["achternaam"] if "achternaam" in _s_exp.columns else ""
                    _part_df = pd.DataFrame({
                        "Voornaam": _vn, "Achternaam": _an, "Brevet": _brevet,
                        "Eet mee": _eat_vals, "Maaltijd": _meal_vals, "Extra mee-eters": _extra_vals
                    })
                    ...
                    # Schrijf naar Excel (zelfde sheet, deelnemers vanaf rij 4)
                    _out = io.BytesIO()
