...
# --- Helper: formaat telefoon "landcode 3cijfers/rest" ---
def _format_phone_landcode_slash(raw: str):
    """
    Normaliseer telefoon naar: <landcode> <XXX>/<rest>.
    Voorbeelden: '32 456/123456', '+32 456/123456', '0044 207/1234567'.
    Accepteert invoer met spaties, streepjes, puntjes; laat enkel cijfers over (behoudt '+' voor display).
    """
    import re as _re
    if not raw:
        return None
    ...
    if len(digits) < 6:
        raise ValueError("Telefoon te kort. Verwacht: landcode + 3 cijfers + rest.")
    # Bepaal landcode-lengte
    cc_len = 0
    if s.startswith('+') or s.startswith('00'):
        for k in (3, 2):
            if len(digits) > k + 3:
                ...
                break
        if cc_len == 0:
            cc_len = 2 if len(digits) >= 5 else 1
    else:
        cc_len = 2 if len(digits) >= 5 else 1
    cc = digits[:cc_len]
    tail = digits[cc_len:]
    if len(tail) < 4:
        raise ValueError("Telefoon te kort na landcode. Verwacht: 3 cijfers + '/' + rest.")
    first3 = tail[:3]
    rest = tail[3:]
    prefix = ('+' if keep_plus else '') + cc
    return f"{prefix} {first3}/{rest}"
...
def page_profiel():
    ...
    with col2:
        ...
        if ("geboortedatum" in _df_cols) or (v_gbd is not None):
            ...
            if _default_date is not None:
                i_gbd = st.date_input("Geboortedatum", value=_default_date, format="DD/MM/YYYY", key="prof_gbd_date", min_value=datetime.date(1900, 1, 1))
            else:
                i_gbd = st.date_input("Geboortedatum", value=None, format="DD/MM/YYYY", key="prof_gbd_date", min_value=datetime.date(1900, 1, 1))
    ...
    disabled = is_readonly() or not uname
        # Normaliseer telefoon
    try:
        if 'i_tel' in locals() and i_tel is not None and str(i_tel).strip():
            i_tel = _format_phone_landcode_slash(i_tel)
    except Exception as _ph_ex:
        st.error(f"Telefoon ongeldig: {_ph_ex}")
        return
