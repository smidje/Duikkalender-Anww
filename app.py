    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        with st.expander(f"{titel}   ·   {badge}", expanded=False):
            ...
            if current_role() in {"admin", "user", "member"} and not is_readonly():
                ...
                # Tekst boven de knoppen

                st.markdown("**Status**")

                # Per-activiteit CSS voor ronde, even brede, gekleurde knoppen

                st.markdown(f"""

                <style>

                #rsvp_yes_{row['id']} .stButton > button {{
                ...
                #rsvp_no_{row['id']} .stButton > button {{
                ...
                with col_yes:
                    ...
                    if st.button("Ik kom", key=f"btn_yes_{row['id']}", use_container_width=True):
                ...
                with col_no:
                    ...
                    if st.button("Ik kom niet", key=f"btn_no_{row['id']}", use_container_width=True):
