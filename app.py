        ...
        if df_del.empty: st.caption("Geen toekomstige activiteiten.")
            ...
            if _act_action == "Verwijderen":
                ...
                if _act_id:
                    ...
                    ec1, ec2 = st.columns([2,1])
                    with ec1:
                        etitel = st.text_input("Titel*", value=str(_row.get("titel") or ""), key="adm_e_title")
                        eomschr = st.text_area("Omschrijving", value=str(_row.get("omschrijving") or ""), key="adm_e_om")
                        eopm = st.text_area("Opmerkingen", value=str(_row.get("opmerkingen") or ""), key="adm_e_opm")
                    with ec2:
                        import datetime
                        try:
                        ...
                        try:
                            ...
                            _t = None
                        edatum = st.date_input("Datum*", value=_d, format="DD/MM/YYYY", key="adm_e_date")
                        etijd = st.time_input("Tijd (optioneel)", value=_t, key="adm_e_time")
                        _pl = plaatsen_list()
                        _loc = _row.get("locatie") or ""
                        _ix = 1 + _pl.index(_loc) if _loc in _pl else 0
                        eloc = st.selectbox("Locatie", ["— kies —"] + _pl, index=_ix, key="adm_e_loc")
                    # Organisator (bewerken)
                    try:
                        _ld2 = leden_list_df()
                        _ld2 = _ld2.dropna(subset=['username']).copy() if isinstance(_ld2, pd.DataFrame) else pd.DataFrame([])
                    ...
                    if not _ld2.empty:
                        ...
                        for _ix2, _r2 in _ld2.iterrows():
                            ...
                            _org_labels2.append(_lbl2); _org_map2[_lbl2] = _u2
                            if _u2 == _cur_org: _def_ix = len(_org_labels2) - 1
                    organisator_sel2 = st.selectbox('Organisator', _org_labels2, index=_def_ix, key="adm_e_org")
                    organisator_user2 = _org_map2.get(organisator_sel2)
                    # Opslaan
