...
# Permissies per gebruiker (fijne rechten bovenop rol)
PERMISSIONS = {
    "create_activity": "Activiteiten aanmaken",
    "edit_activity": "Activiteiten bewerken/verwijderen",
    "manage_signups": "Inschrijvingen beheren",
    "manage_afrekening": "Afrekening markeren als betaald",
    "manage_weekmail": "Wekelijkse weekmail versturen/instellen",
}

PERM_DEFAULTS = {
    # Standaard zoals de app tot nu toe werkte:
    # - Nieuwe activiteit: admin, user, viewer
    "create_activity": {"admin": True, "user": True, "member": False, "viewer": True},
    # - Activiteit bewerken: admin + user
    "edit_activity": {"admin": True, "user": True, "member": False, "viewer": False},
    # - Inschrijvingen beheren: admin + user
    "manage_signups": {"admin": True, "user": True, "member": False, "viewer": False},
    # - Afrekening markeren als betaald: admin, user, viewer
    "manage_afrekening": {"admin": True, "user": True, "member": False, "viewer": True},
    # - Weekmail bedienen: enkel admin
    "manage_weekmail": {"admin": True, "user": False, "member": False, "viewer": False},
}

def _permissions_for_user(user: dict) -> dict:
    """Bepaal effectieve rechten voor een gebruiker op basis van rol + eventuele perm_* kolommen in 'leden'."""
    ...
    for key in PERMISSIONS.keys():
        ...
        if isinstance(override, bool):
            out[key] = bool(override)
        elif isinstance(override, (int, float)) and override in (0, 1):
            out[key] = bool(override)
        else:
            out[key] = bool(PERM_DEFAULTS.get(key, {}).get(role, False))
    return out

def current_permissions() -> dict:
    """Geeft de effectieve permissies voor de huidige gebruiker."""
    return _permissions_for_user(current_user())

def can(perm_key: str) -> bool:
    """Controleer of de huidige gebruiker een bepaalde actie mag uitvoeren.

    Admin mag altijd alles; voor andere rollen worden PERM_DEFAULTS + eventuele perm_* overrides gebruikt.
    """
    role = current_role()
    if role == "admin":
        return True
        ...
        if sel:
            ...
            with st.form("edit_lid_form", clear_on_submit=False):
                ...
                with c1:
                    ...
                    # Rol-selectie met veilige default
                    _cur_role = (row.get("role") or "member")
                    if _cur_role not in ["admin","user","member","viewer"]:
                        _cur_role = "member"
                    role = st.selectbox(
                        "Rol",
                        ["admin", "user", "member", "viewer"],
                        index=["admin", "user", "member", "viewer"].index(_cur_role),
                    )
                with c2:
                ...
                # Rechten per lid (indien permissiekolommen bestaan in 'leden')
                perm_cols = set(df.columns) if isinstance(df, pd.DataFrame) else set()
                with st.expander("Rechten voor dit lid", expanded=False):
                    if perm_cols & {"perm_create_activity","perm_edit_activity","perm_manage_signups","perm_manage_afrekening","perm_manage_weekmail"}:
                        _role_eff = (row.get("role") or "member").lower()
                        if _role_eff not in {"admin","user","member","viewer"}:
                            _role_eff = "member"
                        if "perm_create_activity" in perm_cols:
                            _def_ca = PERM_DEFAULTS.get("create_activity", {}).get(_role_eff, False)
                            perm_values["perm_create_activity"] = st.checkbox(
                                "Mag activiteiten aanmaken",
                                value=bool(row.get("perm_create_activity", _def_ca)),
                                key=f"perm_create_activity_{m_.get('id') or m_.get('username') or m_.get('email')}",
                            )
                        if "perm_edit_activity" in perm_cols:
                            _def_ea = PERM_DEFAULTS.get("edit_activity", {}).get(_role_eff, False)
                            perm_values["perm_edit_activity"] = st.checkbox(
                                "Mag activiteiten bewerken/verwijderen",
                                value=bool(row.get("perm_edit_activity", _def_ea)),
                                key=f"perm_edit_activity_{m_.get('id') or m_.get('username') or m_.get('email')}",
                            )
                        if "perm_manage_signups" in perm_cols:
                            _def_ms = PERM_DEFAULTS.get("manage_signups", {}).get(_role_eff, False)
                            perm_values["perm_manage_signups"] = st.checkbox(
                                "Mag inschrijvingen beheren",
                                value=bool(row.get("perm_manage_signups", _def_ms)),
                                key=f"perm_manage_signups_{m_.get('id') or m_.get('username') or m_.get('email')}",
                            )
                        if "perm_manage_afrekening" in perm_cols:
                            _def_ma = PERM_DEFAULTS.get("manage_afrekening", {}).get(_role_eff, False)
                            perm_values["perm_manage_afrekening"] = st.checkbox(
                                "Mag afrekening markeren als betaald",
                                value=bool(row.get("perm_manage_afrekening", _def_ma)),
                                key=f"perm_manage_afrekening_{m_.get('id') or m_.get('username') or m_.get('email')}",
                            )
                        if "perm_manage_weekmail" in perm_cols:
                            _def_mw = PERM_DEFAULTS.get("manage_weekmail", {}).get(_role_eff, False)
                            perm_values["perm_manage_weekmail"] = st.checkbox(
                                "Mag weekmail versturen/instellen",
                                value=bool(row.get("perm_manage_weekmail", _def_mw)),
                                key=f"perm_manage_weekmail_{m_.get('id') or m_.get('username') or m_.get('email')}",
                            )
                    else:
                        st.caption("Nog geen permissiekolommen (perm_...) in tabel 'leden'.")
            ...
            if save:
                try:
                    ...
                    for _k, _v in perm_values.items():
                        payload[_k] = bool(_v)
        ...
        with col_main:
            with st.expander(f"{titel}   ·   {badge}", expanded=False):
                ...
                with colB:
                    ...
                    # --- Inschrijvingen beheren (alleen admin/user of met permissie) ---
                    if can("manage_signups"):
                ...
                if current_role() in {"admin", "user", "member"} and not is_readonly():
                    ...
                    if st.button('Bewaar mijn keuze', key=f'save_{row["id"]}', type='primary', disabled=(status == '— kies —')):
                        try:
                            ...
                            # --- Beheer activiteit (admin, user of met permissie) ---
                            if can("edit_activity"):
