            ...
            with st.expander(f"{titel}   ·   {badge}", expanded=False):
                ...
                with colB:
                    ...
                    if current_role() in {"admin","user"}:
                        with st.expander("👥 Inschrijvingen beheren", expanded=True):
                            ...
                            if s_all is None or s_all.empty:
                                ...
                                for _, srow in s_all.sort_values(by=["status","username"], ascending=[False, True]).iterrows():
                                    uname = srow.get("username") or "lid"
                                    c1, c2, c3, c4, c5 = st.columns([2,1,1,2,1])
                                    with c1: st.write(uname)
                                    with c2:
