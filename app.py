...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        with col_main:
            # Gekleurde achtergrond over de volledige activiteitenregel (overzicht)

            try:

                _hex = activity_color_get(row.get('id'))

                if _hex:

                    # Volledige regel: hogere balk met negatieve marge zodat de expander erboven ligt

                    st.markdown(

                        f"<div style='background:{_hex};border-radius:10px;height:42px;margin:6px 0 -36px 0;border:1px solid var(--border);'></div>",
                ...
                pass
            # Achtergrondkleur per activiteit (volledige regel in overzicht)
            __has_bg = False
            try:
                _hex = activity_color_get(row.get('id'))
                if _hex:
                    __has_bg = True
                    st.markdown(f"<div style='background:{_hex}; border-radius:10px; padding:4px 8px; margin:6px 0;'>", unsafe_allow_html=True)
            except Exception:
                pass
            # Instelbare achtergrondkleur van de activiteitenregel (expander zelf)
            try:
                _hex = activity_color_get(row.get('id'))
                if _hex:
                    _mk = 'act-' + str(row['id'])
                    ...
                    <div id='{_mk}'></div>
                    <style>
                    /* kleur de expander header (samenvatting) in het *volgende* element */
                    #{_mk} + div [data-testid=\"stExpander\"] summary {
                        background: {_hex} !important;
                        border-radius: 10px !important;
                        color: inherit !important;
                    }
                    #{_mk} + div [data-testid=\"stExpander\"] details {
            ...
            with st.expander(f"{titel}   ·   {badge}", expanded=False):
                ...
                # Admin keuzevak: kleur per activiteit
                if current_role() == 'admin':
                    _opt_map = {
                        'Geen': None,
                        'Cursus (#E35B40)': '#E35B40',
                        'Clubduik (#684EF5)': '#684EF5',
                        'Vergadering (#F5994E)': '#F5994E',
                        'Groepsactiviteit (#DE9AED)': '#DE9AED',
                    }
                    _cur_hex = activity_color_get(row.get('id'))
                    _cur_label = next((k for k,v in _opt_map.items() if v == _cur_hex), 'Geen')
                    _sel = st.selectbox('Kleur voor deze activiteit', list(_opt_map.keys()), index=list(_opt_map.keys()).index(_cur_label), key=f'act_color_sel_{row["id"]}')
                    if st.button('Bewaar kleur', key=f'act_color_save_{row["id"]}', disabled=is_readonly()):
                        activity_color_set(row.get('id'), _opt_map.get(_sel))
                        st.success('Kleur opgeslagen')
                        st.rerun()
...
# --- Per-activiteit kleur mapping ---
def activity_colors_map_get() -> dict:
    try:
        m = asset_meta_get('activity_colors') or {}
...
def activity_colors_map_set(mapping: dict):
    if is_readonly():
        return
    clean = {}
    for k, v in (mapping or {}).items():
        ...
        if vv and not vv.startswith('#'):
            vv = '#' + vv
        clean[kk] = vv
    asset_meta_set('activity_colors', clean)

def activity_color_get(act_id):
    m = activity_colors_map_get()
    return m.get(str(act_id))

def activity_color_set(act_id, hexcolor: str | None):
    if is_readonly():
        return
    m = activity_colors_map_get()
    key = str(act_id)
    if not hexcolor:
        if key in m: m.pop(key, None)
    else:
        x = hexcolor.strip()
        if x and not x.startswith('#'):
            x = '#' + x
        m[key] = x
    activity_colors_map_set(m)
