...
def page_profiel():
    ...
    with col1:
        ...
        if ("duikbrevet" in _df_cols) or (v_brevet is not None):
            ...
            i_brevet = None
    with col2:
        if ("telefoon" in _df_cols) or v_tel:
            i_tel = st.text_input("Telefoon", value=v_tel, key="prof_tel")
        else:
            i_tel = None
        if ("adres" in _df_cols) or v_adres:
            i_adres = st.text_area("Adres", value=v_adres, key="prof_adr")
        else:
            i_adres = None
        if ("geboortedatum" in _df_cols) or (v_gbd is not None):
            # Gebruik date_input; we proberen bestaande waarde te parsen
            _default_date = None
            _raw = v_gbd
            if _raw:
                try:
            ...
            if _default_date is not None:
                i_gbd = st.date_input("Geboortedatum", value=_default_date, format="DD/MM/YYYY", key="prof_gbd_date")
            else:
                i_gbd = st.date_input("Geboortedatum", value=None, format="DD/MM/YYYY", key="prof_gbd_date")
            # Voorbeeld dd/maand/jaar
            try:
                import pandas as _pd
    ...
    if st.button("Bewaar mijn gegevens", type="primary", disabled=disabled, key="prof_save_btn"):
        try:
            ...
            if i_tel is not None:
                payload["telefoon"] = (i_tel or "").strip()
            if i_adres is not None:
                payload["adres"] = (i_adres or "").strip()
