...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        # Tellers incl. extra mee-eters
        yes_count = int((s["status"] == "yes").sum()) if not s.empty else 0
        no_count  = int((s["status"] == "no").sum())  if not s.empty else 0
        extra_sum = 0
        if "opmerking" in s.columns:
            ...
            try: extra_sum = int(s["opmerking"].apply(_extra_from).sum())
            except Exception: extra_sum = 0
        tot_yes = yes_count + extra_sum
        st.markdown(f'<div id="act_{row["id"]}"></div>', unsafe_allow_html=True)
        st.markdown(f"""
        <style>
        #act_{row['id']} ~ div[data-testid='stExpander'] {{ margin-bottom: 0.10rem !important; }}
        #act_{row['id']} ~ div[data-testid='stExpander'] button {{ position: relative; }}
        #act_{row['id']} ~ div[data-testid='stExpander'] button::after {{ content: "   🟢 {tot_yes} / 🔴 {no_count}"; position: absolute; right: 14px; top: 50%; transform: translateY(-50%); }}
        </style>
        """, unsafe_allow_html=True)
        with st.expander(f"{titel}   ·   {badge}", expanded=False):
            ...
            if current_role() in {"admin", "user", "member"} and not is_readonly():
                ...
                # RSVP met vinkvakjes onder elkaar
                yes_key = f"ck_yes_{row['id']}"
                no_key  = f"ck_no_{row['id']}"
                # Voorvullen vanuit bestaande status
                default_yes = (my_status == "yes")
                default_no  = (my_status == "no")
                yes_ck = st.checkbox("Ik kom", value=default_yes, key=yes_key)
                no_ck  = st.checkbox("Ik kom niet", value=default_no, key=no_key)
                # "Ik eet mee" staat eronder, maar telt alleen als je komt
                eating_val = st.checkbox("Ik eet mee", value=prev_eating, key=f"eat_{row['id']}")
                # Bepaal status — als beide aangevinkt, eerst oplossen
                if yes_ck and no_ck:
                    st.warning("Kies maximaal één van: 'Ik kom' of 'Ik kom niet'.")
                    status = "— kies —"
                else:
                    status = "Ik kom" if yes_ck else ("Ik kom niet" if no_ck else "— kies —")
                eating = eating_val if status == "Ik kom" else None
                extra_key = f"eat_extra_{row['id']}"
                extra_people = st.number_input("Extra mee-eters", min_value=0, max_value=20, step=1, value=0, key=extra_key) if (status == "Ik kom" and eating) else 0
                ...
                if st.button("Bewaar mijn keuze", key=f"save_{row['id']}", type="primary", disabled=(status == "— kies —")):
                    try:
                        signup_upsert(activiteit_id=row["id"], username=my_username or None, lid_id=my_lid_id or None,
                                      ...
                                      opmerking=(((remark_val or "").strip() + (f" [extra mee-eters: {extra_people}]" if extra_people else "")).strip() if status == "Ik kom" else None))
