...
def leden_upsert(payload: dict):
    ...
    # NOTE: previously on_conflict="email" — if e-mail differs but username already exists,
    # Postgres raised duplicate key error on username. Include username in on_conflict so
    # upsert will update when either email OR username conflicts.
    run_db(lambda c: c.table("leden").upsert(p, on_conflict="email,username").execute(), what="leden upsert")
