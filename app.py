...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        st.markdown(f"""
        <style>
        #act_{row['id']} ~ div[data-testid='stExpander'] {{ margin-bottom: 0.15rem !important; }}
        #act_{row['id']} ~ div[data-testid='stExpander'] button::after {{ content: "🟢 {yes_count} / 🔴 {no_count}"; position: absolute; right: 12px; top: 50%; transform: translateY(-50%); }}
        </style>
        """, unsafe_allow_html=True)
