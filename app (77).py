...
def asset_get(key: str) -> bytes | None:
    # Get JPEG from 'app_assets' (image_b64); tries SERVICE-ROLE REST first (bypass RLS), then DB/session fallback.
        ...
        try:
            ...
            if r.status_code < 400:
                ...
                if rows and rows[0].get("image_b64"):
                    ...
                    try:
                        return _b64.b64decode(rows[0]["image_b64"])
                    except Exception:
                        pass
        except Exception:
            pass
...
def asset_set(key: str, file_bytes: bytes | None, updated_by: str | None = None):
    # Save JPEG to 'app_assets'; tries SERVICE-ROLE REST first (bypass RLS), then DB/session fallback.
    if is_readonly():
