...
def page_leden():
    ...
    # Export-knop naar Excel voor alle ingelogde gebruikers
    try:
        today = datetime.date.today()
        titel = f"Ledenlijst {today.strftime('%d/%m/%Y')}"
        # Helper om kolommen veilig op te halen
        n_rows = len(df)
        def _col(name: str):
        ...
        exp_df = pd.DataFrame({
            ...
            "email": _col("email"),
            "adres compleet": _col("adres"),
            "geboortedatum": pd.to_datetime(_col("geboortedatum"), errors="coerce").dt.strftime("%d/%m/%Y"),
            "brevet": _col("duikbrevet"),
        })
        out = io.BytesIO()
        with pd.ExcelWriter(out, engine="openpyxl") as writer:
            exp_df.to_excel(writer, index=False, sheet_name="Leden", startrow=1)
            ws = writer.book["Leden"]
            ws.cell(row=1, column=1, value=titel)
        st.download_button(
            "⬇️ Export leden naar Excel",
            data=out.getvalue(),
            file_name=f"ledenlijst_{today.strftime('%Y%m%d')}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )
    except Exception as _ex:
        st.warning(f"Export naar Excel mislukt: {_ex}")
...
# --- Helpers: adres & telefoon parsing/formattering ---e')}")
    st.write(f"**Rol:** {u.get('role')}")
    if u.get("email"): st.write(f"**E-mail:** {u.get('email')}")
    st.divider(); st.subheader("Wekelijkse mail")
    row = leden_get_by_username(current_username())
...
def main():
    ...
    role = current_role()
    if role == "admin":
        tabs = st.tabs(["Activiteiten","Duiken invoeren","Overzicht","Afrekening","Beheer","Mijn profiel","Leden"])
        ...
        tabs = st.tabs(["Afrekening","Mijn profiel","Leden"])
