...
# --- Login audit logboek (enkel zichtbaar voor Admin Dirk Verbraeken) ---
LOGIN_AUDIT_KEY = "login_audit"

def _norm_txt(x: str) -> str:
    return (x or "").strip().lower()

def is_dirk_admin() -> bool:
    """True enkel voor de bedoelde admin (Dirk Verbraeken)."""
    if current_role() != "admin":
        return False
    un = _norm_txt(current_username())
    em = _norm_txt(current_email())

    # Veelgebruikte varianten van username/email (tolerant, maar strikt admin-only)
    if un in {"dirk verbaeken", "dirkverbraeken", "dirk_verbraeken", "d.verbraeken", "dirk.verbraeken"}:
        return True
    if em in {"dirk.verbraeken@local"}:
        return True

    # Controle via leden-tabel (voornaam/achternaam) — soft (mag app niet blokkeren)
    try:
        res = sb.table("leden").select("voornaam, achternaam").eq("username", current_username()).limit(1).execute()
        rows = getattr(res, "data", None) or []
        row = rows[0] if rows else {}
        fn = _norm_txt(row.get("voornaam") or "")
        ln = _norm_txt(row.get("achternaam") or "")
        if fn == "dirk" and ln == "verbraeken":
            return True
    except Exception:
        pass
...
def login_audit_read() -> list[dict]:
    """Leest het login-logboek uit app_assets (key=LOGIN_AUDIT_KEY)."""
    try:
        b = asset_get_bytes(LOGIN_AUDIT_KEY)
        if not b:
            return []
...
def login_audit_append(entry: dict, *, max_entries: int = 2000):
    """Voegt een entry toe (best-effort)."""
    if is_readonly():
        return
    try:
        log = login_audit_read()
        log.append(entry or {})
        if len(log) > max_entries:
            log = log[-max_entries:]
        payload = json.dumps(log, ensure_ascii=False).encode("utf-8")
        # Gebruik dezelfde storage-laag als banners (app_assets)
        asset_set_bytes(LOGIN_AUDIT_KEY, payload, updated_by=(current_username() or current_email() or None))
    except Exception:
        # Audit mag nooit de app blokkeren
        pass

def login_audit_clear():
    if is_readonly():
        raise Exception("Read-only modus")
    asset_set_bytes(LOGIN_AUDIT_KEY, b"[]", updated_by=(current_username() or current_email() or None))
...
def signup_meta_strip(remark: str | None) -> str:
    """Verwijder door de app toegevoegde meta uit de opmerking zodat het tekstveld 'proper' blijft.
    Meta die we strippen:
    - [extra mee-eters: N]
    - [Mee-eters] blok (tot einde, aangezien we dit altijd op het einde plaatsen)
    """
    ...
    try:
        import re as _re
        r = _re.sub(r'\s*\[extra\s*mee-eters\s*:\s*\d+\]\s*', ' ', r, flags=_re.I)
    except Exception:
        pass
    # Verwijder [Mee-eters] blok (case-insensitive) vanaf de marker tot einde
    low = r.lower()
    marker = "[mee-eters]"
    pos = low.find(marker)
    if pos != -1:
...
def signup_meta_extract_extra_people(remark: str | None) -> int:
    """Lees aantal extra mee-eters uit een vrije opmerking (robust: met/zonder [] en met variabele spaties)."""
    if not isinstance(remark, str) or not remark.strip():
        return 0
    try:
        import re as _re
        m = _re.search(r'extra\s*mee-eters\s*:\s*(\d+)', remark, _re.I)
        return int(m.group(1)) if m else 0
    except Exception:
        return 0

def signup_meta_extract_meeeters(remark: str | None) -> list[dict]:
    """Parseert het [Mee-eters] blok en geeft een lijst terug: [{'name':..., 'meal':...}, ...]."""
    if not isinstance(remark, str) or not remark.strip():
        return []
    r = str(remark)
    low = r.lower()
    ...
    if pos == -1:
        return []
    block = r[pos + len(marker):].strip()
    if not block:
        return []
    # Neem alle regels tot einde (we plaatsen dit blok altijd op het einde)
    lines = [ln.strip() for ln in block.splitlines() if ln.strip()]
    out: list[dict] = []
    try:
        import re as _re
        rx = _re.compile(r'^\s*(\d+)\)\s*Naam\s*:\s*(.*?)(?:\s*;\s*Maaltijd\s*:\s*(.*))?\s*$', _re.I)
        for ln in lines:
            m = rx.match(ln)
            if not m:
                # tolerant: probeer "naam — maaltijd"
                if "—" in ln:
                    a, b = ln.split("—", 1)
                    out.append({"name": a.strip(), "meal": b.strip()})
                else:
                    out.append({"name": ln.strip(), "meal": None})
                continue
            name = (m.group(2) or "").strip()
            meal = (m.group(3) or "").strip() or None
            out.append({"name": name, "meal": meal})
    except Exception:
        return []
    return out

def signup_meta_build_meeeters_block(meeeters: list[dict], meal_opts: list[str] | None = None) -> str:
    """Bouwt het [Mee-eters] blok in een stabiel, parseerbaar formaat."""
    if not meeeters:
        return ""
    meal_opts = meal_opts or []
    out = ["[Mee-eters]"]
    for i, p in enumerate(meeeters, start=1):
        name = (p.get("name") if isinstance(p, dict) else "") or ""
        name = str(name).strip()
        meal = (p.get("meal") if isinstance(p, dict) else None)
        meal = (str(meal).strip() if isinstance(meal, str) else "")
        meal = meal if meal and (not meal_opts or meal in meal_opts) else (meal if meal else "")
        if meal:
            out.append(f"{i}) Naam: {name}; Maaltijd: {meal}")
        else:
            out.append(f"{i}) Naam: {name}")
    return "\n".join(out).strip()

def signup_meta_meeeters_summary(remark: str | None) -> str:
    """Korte samenvatting voor tabellen/exports."""
    mee = signup_meta_extract_meeeters(remark)
    if not mee:
        return ""
    parts = []
    for i, p in enumerate(mee, start=1):
        nm = (p.get("name") or "").strip() if isinstance(p, dict) else ""
        ml = (p.get("meal") or "").strip() if isinstance(p, dict) else ""
        if ml:
            parts.append(f"{i}) {nm} — {ml}".strip())
        else:
            parts.append(f"{i}) {nm}".strip())
...
def page_login_username():
    ...
    try:
        login_audit_append({
            "ts_utc": dt.utcnow().isoformat(),
            "username": user.get("username"),
            "email": (user.get("email") or ""),
            "role": (user.get("role") or ""),
            "login_input": (login or "").strip(),
            "build": APP_BUILD,
        })
    except Exception:
        pass
...
def page_logboek():
    appbar("logboek")
    if not is_dirk_admin():
        st.error("Onvoldoende rechten.")
        return

    st.header("Logboek — ingelogde gebruikers")

    rows = login_audit_read()
    if not rows:
        st.info("Nog geen logins geregistreerd.")
        return

    df = pd.DataFrame(rows)

    # Normaliseer kolommen
    for col in ["ts_utc","username","email","role","build","login_input"]:
        if col not in df.columns:
            df[col] = None
    ...
    with c1:
        start = st.date_input("Van", value=(datetime.date.today() - datetime.timedelta(days=30)), format="DD/MM/YYYY", key="log_van")
    with c2:
        end = st.date_input("Tot", value=datetime.date.today(), format="DD/MM/YYYY", key="log_tot")
    with c3:
        q = st.text_input("Filter (username/email bevat)", value="", key="log_q")

    m = pd.Series([True] * len(df))
    if "ts_utc" in df.columns and start and end:
        try:
            m &= df["ts_utc"].dt.date.between(start, end)
        except Exception:
            pass
    if q:
        qq = q.strip().lower()
        m &= (df["username"].astype(str).str.lower().str.contains(qq, na=False) |
              df["email"].astype(str).str.lower().str.contains(qq, na=False))

    view = df.loc[m].copy()
    view = view.sort_values("ts_utc", ascending=False, na_position="last")

    st.caption(f"Aantal entries: {len(view)} (van totaal {len(df)})")
    st.dataframe(view, use_container_width=True, hide_index=True)

    # Export
    try:
        csv = view.to_csv(index=False).encode("utf-8")
        st.download_button("⬇️ Download CSV", data=csv, file_name="login_logboek.csv", mime="text/csv", key="log_export_csv")
    except Exception:
        pass
    ...
    st.subheader("Beheer")
    if is_readonly():
        st.warning("Read-only modus actief — logboek kan niet gewist worden.")
    else:
        if st.button("Logboek wissen", type="secondary", key="log_clear_btn"):
            try:
                login_audit_clear()
...
def page_profiel():
    ...
    _raw_aantal = row.get("aantal_duiken") if ("aantal_duiken" in _df_cols or "aantal_duiken" in row) else 0
    try:
        v_aantal_duiken = int(_raw_aantal if _raw_aantal not in (None, "") else 0)
    except Exception:
        v_aantal_duiken = 0

    # Parse bestaande adres naar losse velden
    try:
    ...
    with col1:
        ...
        i_aantal_duiken = st.number_input("Aantal duiken", min_value=0, step=1, value=v_aantal_duiken, key="prof_aantal_duiken")
        if ("duikbrevet" in _df_cols) or (v_brevet is not None):
            ...
            i_brevet = None
    with col2:
        # Telefoon als één veld
        if ("telefoon" in _df_cols) or (v_tel is not None):
            i_tel = st.text_input("Telefoon", value=v_tel, key="prof_tel")
        else:
            i_tel = None
    ...
    if st.button("Bewaar mijn gegevens", type="primary", disabled=disabled, key="prof_save_btn"):
        try:
            payload = {
                ...
                "aantal_duiken": int(i_aantal_duiken if i_aantal_duiken is not None else 0),
            }
            if i_brevet is not None:
                payload["duikbrevet"] = None if i_brevet == "(geen)" else canon_brevet(i_brevet)

            # Telefoon opslaan (één veld)
            if i_tel is not None:
                payload["telefoon"] = (i_tel or "").strip() or None
...
def page_leden():
    ...
    try:
        ...
        export_df = pd.DataFrame({
            ...
            "Aantal duiken": _col("aantal_duiken"),
        })
...
def page_ledenbeheer():
    ...
    df = leden_list_df()
    if not df.empty:
        cols = ["voornaam","achternaam","email","username","role","duikbrevet","aantal_duiken","opt_in_weekly","actief"]
...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        try:
            ...
            _ldu = leden_df[['username','voornaam','achternaam','aantal_duiken']].dropna(how='all') if isinstance(leden_df, pd.DataFrame) else None
        ...
        with col_main:
            with st.expander(f"{titel}   ·   {badge}", expanded=False):
                ...
                with colA:
                    ...
                    if coming.empty: st.caption("Nog niemand.")
                        ...
                        for _, ss in coming.iterrows():
                            ...
                            _duik_raw = ss.get('aantal_duiken', 0)
                            try:
                                _duik = int(_duik_raw) if _duik_raw not in (None, '') else 0
                            except Exception:
                                _duik = 0
                            st.write(f"- {_nm}{(' · ' + _tstr) if _tstr else ''}{meal} · #duiken: {_duik}")
                with colB:
                    ...
                    if current_role() in {"admin","user"}:
                        with st.expander("👥 Inschrijvingen beheren", expanded=False):
                            ...
                            # Mee-eters: naam + maaltijdkeuze (optioneel) bij extra mee-eters
                            add_meeeters = []
                            try:
                                _n_extra = int(add_extra or 0)
                            except Exception:
                                _n_extra = 0
                            if add_status == "Komt" and add_eat and _n_extra > 0:
                                st.markdown("**Mee-eters (optioneel)**")
                                for _i in range(_n_extra):
                                    _nm = st.text_input(f"Naam mee-eter {_i+1}", value="", key=f"addx_name_{row['id']}_{add_user}_{_i}")
                                    _meal = None
                                    if meal_opts:
                                        _sel = st.selectbox(f"Maaltijd mee-eter {_i+1}", ["— kies —"] + meal_opts, index=0, key=f"addx_meal_{row['id']}_{add_user}_{_i}")
                                        _meal = None if _sel == "— kies —" else _sel
                                    add_meeeters.append({"name": _nm, "meal": _meal})

                            if st.button("Toevoegen", key=f"addbtn_{row['id']}", disabled=(add_user == "— kies —" or is_readonly())):
                                try:
                                    opm = None
                                    if add_status == "Komt" and add_eat and _n_extra > 0:
                                        opm = f"[extra mee-eters: {_n_extra}]"
                                        block = signup_meta_build_meeeters_block(add_meeeters, meal_opts=meal_opts)
                                        if block:
                                            opm = (opm + "\n" + block).strip()
                                    signup_upsert(activiteit_id=row["id"], username=add_user, lid_id=None,
                                                  status=("yes" if add_status == "Komt" else "no"),
                                                  eating=(add_eat if add_status == "Komt" else None),
                                                  meal_choice=(add_meal if add_eat and add_status == "Komt" else None),
                                                  opmerking=opm)
                ...
                if current_role() in {"admin", "user", "member"} and not is_readonly():
                    ...
                    prev_eating, prev_meal, prev_remark_raw = False, None, None
                    if myrow is not None and not myrow.empty:
                        ...
                        pr = myrow.iloc[0].get('opmerking'); prev_remark_raw = pr if isinstance(pr, str) else None
                    prev_extra = signup_meta_extract_extra_people(prev_remark_raw)
                    prev_meeeters = signup_meta_extract_meeeters(prev_remark_raw)
                    prev_remark = signup_meta_strip(prev_remark_raw)
                    ...
                    try:
                        extra_people = int(extra_people or 0)
                    except Exception:
                        extra_people = 0
                    ...
                    # Mee-eters: naam + maaltijdkeuze per persoon (optioneel)
                    meeeters = []
                    if status == 'Ik kom' and eating and extra_people > 0:
                        st.markdown("**Mee-eters (optioneel)**")
                        for _i in range(int(extra_people)):
                            _def_name = (prev_meeeters[_i].get("name") if _i < len(prev_meeeters) and isinstance(prev_meeeters[_i], dict) else "") or ""
                            _name = st.text_input(f"Naam mee-eter {_i+1}", value=str(_def_name), key=f"exname_{row['id']}_{_i}")
                            _meal = None
                            if meal_opts:
                                _def_meal = (prev_meeeters[_i].get("meal") if _i < len(prev_meeeters) and isinstance(prev_meeeters[_i], dict) else None)
                                _def_ix = 0
                                if isinstance(_def_meal, str) and _def_meal in meal_opts: _def_ix = meal_opts.index(_def_meal) + 1
                                _sel = st.selectbox(f"Maaltijd mee-eter {_i+1}", ['— kies —'] + meal_opts, index=_def_ix, key=f"exmeal_{row['id']}_{_i}")
                                _meal = None if _sel == '— kies —' else _sel
                            meeeters.append({"name": _name, "meal": _meal})

                    # Opmerking (enkel vrije tekst; meta wordt automatisch toegevoegd bij bewaren)
                    remark_val = st.text_area('Opmerking (optioneel)', value=(prev_remark or ''), key=f'remark_{row["id"]}')

                    if st.button('Bewaar mijn keuze', key=f'save_choice_{row["id"]}', type='primary', disabled=(status == '— kies —')):
                        try:
                            base_remark = signup_meta_strip(remark_val or '')
                            opm_final = base_remark
                            if status == 'Ik kom':
                                # Voeg meta opnieuw toe: extra mee-eters + (optioneel) details
                                meta_parts = []
                                if eating and int(extra_people or 0) > 0:
                                    meta_parts.append(f"[extra mee-eters: {int(extra_people)}]")
                                if meta_parts:
                                    opm_final = (opm_final + (" " if opm_final else "") + " ".join(meta_parts)).strip()
                                if eating and int(extra_people or 0) > 0:
                                    block = signup_meta_build_meeeters_block(meeeters, meal_opts=meal_opts)
                                    if block:
                                        opm_final = (opm_final + ("\n" if opm_final else "") + block).strip()
                            else:
                                opm_final = None
                ...
                try:
                    ...
                    _meeeters = _s1.get("opmerking", pd.Series([""]*_N) if _N else pd.Series([], dtype=object)).apply(signup_meta_meeeters_summary) if _N else pd.Series([], dtype=object)
                    ...
                    _part_df = pd.DataFrame({
                        ...
                        "Eet mee": _eat, "Maaltijd": _meal, "Extra mee-eters": _extra, "Mee-eters": _meeeters
...
def main():
    ...
    if role == "admin":
        if is_dirk_admin():
            tabs = st.tabs(["Activiteiten","Duiken invoeren","Overzicht","Afrekening","Beheer","Logboek","Mijn profiel"])
            ...
            with tabs[5]: page_logboek()
            with tabs[6]: page_profiel()
        else:
