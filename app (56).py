...
def page_profiel():
    ...
    if st.button("Bewaar mijn gegevens", key="prof_save_btn"):
        try:
            fields = {
                "voornaam": v_voor.strip(),
                "achternaam": v_naam.strip(),
                "email": v_email.strip(),
                "brevet": v_brevet.strip(),
                "telefoon": v_tel.strip(),
                "adres": v_adres.strip(),
                "geboortedatum": v_geboorte.strip(),
            }
...
def page_leden():
    """Overzicht van leden. Voor admin: bewerken. Voor user/member: alleen lezen.
       Kolommen 'username' en 'role' enkel zichtbaar voor admin.
    """
    st.header("Leden")
    ...
    # Zoekveld (filter op alle zichtbare kolommen)
    ...
    # Geboortedatum tonen als dd/maand/jaar (bv. 05/oktober/1992)
    def _fmt_nl_date(val):
        try:
            dt = pd.to_datetime(val, errors="coerce")
            if pd.isna(dt):
    ...
    if rol == "admin":
        ...
        st.caption("Wil je je eigen gegevens aanpassen? Dat kan bij **Mijn profiel**.")
