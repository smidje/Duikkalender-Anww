...
# --- JPEG banner assets (DB + session fallback) ---
def asset_get(key: str) -> bytes | None:
    """Fetch JPEG bytes from 'app_assets' (image_b64). Falls back to session state if DB unavailable."""
    try:
...
def asset_set(key: str, file_bytes: bytes | None, updated_by: str | None = None):
    """Save JPEG into 'app_assets' (key,image_b64,updated_by,updated_ts). Falls back to session if DB blocked."""
    if is_readonly():
    ...
    if file_bytes is None:
        ...
        try:
            ...
            st.session_state[f"_asset_{key}"] = None
        return
    img_b64 = base64.b64encode(file_bytes).decode("ascii")
    payload = {"key": key, "image_b64": img_b64, "updated_by": (updated_by or None), "updated_ts": dt.utcnow().isoformat()}
    try:
        run_db(lambda c: c.table("app_assets").upsert(payload, on_conflict="key").execute(), what=f"app_assets upsert {key}")
    except Exception:
        st.session_state[f"_asset_{key}"] = file_bytes
...
def appbar(tag: str):
    # BANNER boven menubalk (admin beheerbaar)
    try:
        _bn = asset_get('banner_top')
    except Exception:
        _bn = None
    if _bn:
...
def page_login_username():
    # BANNER op loginpagina (admin beheerbaar)
    try:
        _lb = asset_get('banner_login')
    except Exception:
        _lb = None
    if _lb:
        st.image(_lb, use_container_width=True)
    ...
    with st.expander('🖼️ Site afbeeldingen (admin)', expanded=False):
        ...
        with colA:
            ...
            if cA1.button('Bewaar banner', disabled=(is_readonly() or _upA is None)):
                try:
                    asset_set('banner_top', (_upA.read() if _upA else None), updated_by=(current_username() or current_email()))
                    st.success('Banner opgeslagen.'); st.rerun()
                except Exception as e:
                    st.error(f'Mislukt: {e}')
            if cA2.button('Verwijder banner', disabled=is_readonly()):
                try:
                    asset_set('banner_top', None, updated_by=(current_username() or current_email()))
                    st.success('Banner verwijderd.'); st.rerun()
                except Exception as e:
                    st.error(f'Mislukt: {e}')
        with colB:
            st.markdown('**Banner op loginpagina (JPEG)**')
            _curB = asset_get('banner_login')
            ...
            if cB1.button('Bewaar login-banner', disabled=(is_readonly() or _upB is None)):
                try:
                    asset_set('banner_login', (_upB.read() if _upB else None), updated_by=(current_username() or current_email()))
                    st.success('Login-banner opgeslagen.'); st.rerun()
                except Exception as e:
                    st.error(f'Mislukt: {e}')
            if cB2.button('Verwijder login-banner', disabled=is_readonly()):
                try:
                    asset_set('banner_login', None, updated_by=(current_username() or current_email()))
                ...
                if current_role() in {"admin", "user", "member"} and not is_readonly():
                    ...
                    if st.button('Bewaar mijn keuze', key=f'save_{row["id"]}', type='primary', disabled=(status == '— kies —')):
                        try:
                            ...
                            if current_role() in {"admin","user"}:
                                with st.expander("⚙️ Beheer activiteit", expanded=False):
