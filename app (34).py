...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        st.markdown(f"""
        <style>
        #act_{row['id']} + div {{ margin-bottom: 0.25rem !important; }}
        #act_{row['id']} + div .st-expanderHeader p {{ position: relative; }}
        #act_{row['id']} + div .st-expanderHeader p::after {{ content: "   🟢 {tot_yes} / 🔴 {no_count}"; position: absolute; right: 10px; top: 0; }}
        </style>
        """, unsafe_allow_html=True)
