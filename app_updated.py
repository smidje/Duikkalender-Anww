...
def page_profiel():
    ...
    v_aantal = int(row.get("aantal_duiken") or 0) if ("aantal_duiken" in _df_cols or "aantal_duiken" in row) else 0
    ...
    with col1:
        ...
        if ("duikbrevet" in _df_cols) or (v_brevet is not None):
            ...
            i_brevet = None
        # Aantal duiken (door lid zelf te wijzigen)
        i_aantal = st.number_input("Aantal duiken", min_value=0, step=1, value=int(v_aantal or 0), key="prof_aantal_duiken")
    ...
    if st.button("Bewaar mijn gegevens", type="primary", disabled=disabled, key="prof_save_btn"):
        try:
            ...
            payload["aantal_duiken"] = int(i_aantal or 0)
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        try:
            ...
            if isinstance(s, pd.DataFrame) and not s.empty and _ldu is not None and not _ldu.empty:
                ...
                if 'aantal_duiken' not in s.columns:
                    s['aantal_duiken'] = 0
                ...
                with colA:
                    ...
                    if coming.empty: st.caption("Nog niemand.")
                        ...
                        for _, ss in coming.iterrows():
                            ...
                            st.write(f"- {_nm}{(' · ' + _tstr) if _tstr else ''}{meal} · #duiken: {int(ss.get('aantal_duiken') or 0)}")
                with colB:
