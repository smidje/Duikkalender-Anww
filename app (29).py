    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        with st.expander(f"{titel}   ·   {badge}", expanded=False):
            ...
            if current_role() in {"admin", "user", "member"} and not is_readonly():
                ...
                init_index = 0 if my_status is None else (1 if my_status == "yes" else 2)
                # Vervangt de keuzebalk door 2 knopjes naast elkaar
                _status_key = f"st_{row['id']}"
                if _status_key not in st.session_state:
                    st.session_state[_status_key] = ("Ik kom" if my_status == "yes" else ("Ik kom niet" if my_status == "no" else "— kies —"))
                # Tekst boven/naast de knoppen

                st.markdown(f'<div id="rsvp_row_{row["id"]}"></div>', unsafe_allow_html=True)
                col_yes, col_no = st.columns(2)

                with col_yes:

                    st.markdown('<div class="rsvp-label"><strong>Ik kom</strong></div>', unsafe_allow_html=True)

                    st.markdown(f'<div id="rsvp_yes_{row["id"]}">', unsafe_allow_html=True)

                    if st.button(" ", key=f"btn_yes_{row['id']}"):

                        st.session_state[_status_key] = "Ik kom"

                    st.markdown("</div>", unsafe_allow_html=True)

                with col_no:

                    st.markdown('<div class="rsvp-label"><strong>Ik kom niet</strong></div>', unsafe_allow_html=True)

                    st.markdown(f'<div id="rsvp_no_{row["id"]}">', unsafe_allow_html=True)

                    if st.button(" ", key=f"btn_no_{row['id']}"):

                        st.session_state[_status_key] = "Ik kom niet"

                    st.markdown("</div>", unsafe_allow_html=True)

                # CSS: standaard kleine ronde witte blokken (7mm); highlight bij selectie

                st.markdown(f"""
<style>
#rsvp_row_{row['id']} + div[data-testid="stHorizontalBlock"] {{{{ gap: 0.25rem !important; }}}}
#rsvp_row_{row['id']} + div[data-testid="stHorizontalBlock"] [data-testid="column"] {{{{ padding-left: 0 !important; padding-right: 0 !important; }}}}
#rsvp_row_{row['id']} .rsvp-label {{{{ text-align: center; display: block; margin-bottom: 2px; }}}}
#rsvp_yes_{row['id']} .stButton > button, #rsvp_no_{row['id']} .stButton > button {{{{
  width: 7mm !important; height: 7mm !important; min-width: 7mm !important;
  padding: 0 !important; line-height: 7mm !important; font-size: 0 !important;
...
""", unsafe_allow_html=True)
                ...
                if st.session_state.get(_status_key) == "Ik kom":

                    st.markdown(f"""

                    <style>

                    #rsvp_yes_{row['id']} .stButton > button {{ background: var(--success) !important; border-color: var(--success) !important; color: #fff !important; }}

                    </style>

                    """, unsafe_allow_html=True)

                elif st.session_state.get(_status_key) == "Ik kom niet":

                    st.markdown(f"""

                    <style>

                    #rsvp_no_{row['id']} .stButton > button {{ background: var(--error) !important; border-color: var(--error) !important; color: #fff !important; }}
                ...
                if st.button("Bewaar mijn keuze", key=f"save_{row['id']}", type="primary", disabled=(status == "— kies —")):
                    try:
                        ...
                        # --- Admin: activiteit bewerken & deelnemers beheren ---
                        if current_role() == "admin":
