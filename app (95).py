...
def appbar(tag: str):
    # BANNER naast menubalk (onderkant uitlijnen met knoppen)
    try:
    ...
    _html = ""
    if _bn_bytes:
        _html = _banner_html(_bn_bytes, _bn_meta) or ""
    if _html:
        c0, c1, c2, c3 = st.columns([4, 5, 3, 2])
        with c0:
            st.markdown(f"<div style='display:flex;align-items:flex-end;height:100%'>{_html}</div>", unsafe_allow_html=True)
        with c1:
            st.markdown("<div style='display:flex;align-items:flex-end;height:100%'><strong>ANWW Duikapp</strong></div>", unsafe_allow_html=True)
        with c2:
            st.markdown(f"<div style='display:flex;align-items:flex-end;height:100%'><div class='badge'>{current_username() or current_email()} · {current_role()} · Build {APP_BUILD}</div></div>", unsafe_allow_html=True)
        with c3:
        ...
        with col2:
            st.markdown(f"<div class='badge'>{current_username() or current_email()} · {current_role()} · Build {APP_BUILD}</div>", unsafe_allow_html=True)
        with col3:
            if st.button("Uitloggen", key=f"logout_{tag}"):
                st.session_state.pop("auth_user", None)
                st.rerun()
...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        try:
            ...
            if _org_lbl:
                titel = f"{row['titel']} — {_org_lbl} — {pd.to_datetime(row['datum']).strftime('%d/%m/%Y')}"
        except Exception:
            pass
        tstr = None
        if row.get('tijd'):
    ...
    if current_role() == "admin":
        st.divider(); st.subheader("Activiteiten bewerken / verwijderen")
        ...
        if df_del.empty: st.caption("Geen toekomstige activiteiten.")
            ...
            _act_action = st.radio("Actie", ["Bewerken", "Verwijderen"], horizontal=True, key="act_admin_action")
            if _act_action == "Verwijderen":
                ...
                _edit_choice = st.selectbox("Kies activiteit om te bewerken", options, key="mg_edit_select")
                _act_id = id_map.get(_edit_choice) if _edit_choice else None
                # Reset eerdere edit-veldstate bij selectie van een andere activiteit
                try:
                    for _k in list(st.session_state.keys()):
                        if _k.startswith('adm_e_') and not _k.endswith(f'_{_act_id}'):
                ...
                if _act_id:
                    ...
                    if _row is None:
                        try:
                            ...
                            _row = {}
                    st.markdown("**Bewerk geselecteerde activiteit**")
                    ec1, ec2 = st.columns([2,1])
                    with ec1:
                    ...
                    organisator_user2 = _org_map2.get(organisator_sel2)
                    # Opslaan
                    if st.button("Opslaan wijzigingen", key="adm_e_save", type="primary", disabled=is_readonly()):
                        try:
                            payload = {
                                "titel": etitel.strip(),
                                "omschrijving": (eomschr or "").strip(),
                            ...
                            st.success("Activiteit opgeslagen."); st.rerun()
                        except Exception as ex:
                            st.error(f"Opslaan mislukt: {ex}")
